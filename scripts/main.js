...
function hideDetails(){
    'use strict';
    document.body.classList.add(HIDDDEN_DETAIL_CLASS);
}

function showDetails(){
    'use strict';
    var frame = document.querySelector(DETAIL_FRAME_SELECTOR);
    document.body.classList.remove(HIDDDEN_DETAIL_CLASS);
...
function ButtonNext(button){
    ...
    button.addEventListener('click', function(event){
        ...
        if(long <= array.length-1){
            setDetailsFromThumb(array[long+1]);
            showDetails();
            long++;
        }
